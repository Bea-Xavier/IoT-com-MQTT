...
Esta é uma aplicação mobile desenvolvida em **React Native com Expo** para monitorar e controlar dispositivos de uma casa inteligente em tempo real. O app se comunica com sensores e atuadores físicos (ou simulados) através do protocolo **MQTT**, utilizando o **HiveMQ** como broker na nuvem, e persiste o histórico de dados localmente no dispositivo com **AsyncStorage**.

---

## 📋 Descrição do Projeto

O aplicativo permite visualizar em tempo real a temperatura e umidade do ambiente, além de controlar o estado de uma lâmpada remotamente. Todas as mensagens recebidas do broker são salvas automaticamente no dispositivo, e o usuário pode consultar o histórico completo de eventos a qualquer momento — mesmo sem conexão.
...
💠 **Histórico local persistente** — todas as mensagens recebidas são salvas automaticamente no dispositivo;

💠 **Tela de histórico** com exibição de tópico, valor e horário de cada registro;

💠 **Limpeza do histórico** com confirmação via modal;
...
```bash
git clone [https://github.com/seu-usuario/seu-repositorio.git](https://github.com/Bea-Xavier/IoT-com-MQTT.git)
cd IoT-com-MQTT
```
...
├── App.js                        # Ponto de entrada, conexão MQTT e navegação
│
└── src/
    ├── components/
    ...
    │   └── HistoryScreen.js      # Tela de histórico de mensagens
    │
    └── services/
        ├── mqttService.js        # Classe de conexão, subscribe e publish MQTT
        └── historyService.js     # Persistência local com AsyncStorage
```
